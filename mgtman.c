...
void LoadFile(char * filename) {
  int i, t, s, h, length, execute, maxdtrack, ss, tt;
  unsigned char * found;
  ...
  // Make SAM filename from filename to 10 chars with spaces, add NULL
  ...
  if (found == NULL) {
    ...
    if (file != NULL) {
      ...
      execute = * (found + 243);
      execute += 256 * * (found + 244);
      execute += 16384 * ( * (found + 242) & 31);
      execute -= 32768; // take away offset

      if (execute != 540671) printf("Execute %d ", execute);
